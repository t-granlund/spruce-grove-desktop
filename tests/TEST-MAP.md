...
|---|---|---|---|---|
| Unit + plugin + tool tests | `dev/SPRUCE-GROVE-OS/tests/` (the CLI repo) | **356 test files** (8,119 tests) | Every tool, hook, plugin tier, i18n catalog (CI-audited), session storage, model factory (incl. the `custom_openai` branch the Synthetic rig rides) | `uv run pytest tests/` in the fork |
...
| Running-now panel | `tests/test_desktop_ui.py` | Sidebar reflects `--console --json` verbatim: 3 mock sessions render with pid + age, the orphan is flagged, the count badge matches, clicking a named session attaches with its own cwd (`grove_acp_start` resume), and an unreachable CLI shows "view unavailable" rather than falsely claiming none are running. |
| Doc convention discovery | `src-tauri/src/project.rs` (unit tests) | New `ADR-007-*.md` / `BRD.md` / `PIPELINE.md` surface with no code change; a non-governance doc (`random-notes.md`) does NOT; absent search dirs yield nothing rather than erroring; vendored trees (`node_modules`, `target`) are never recursed into; results are sorted and deduped. |
| Showcase packet | `tests/test_packet.py` | The send-one-file deliverable is genuinely one file: zero external refs, assets actually inlined, byte-identical to a fresh build (`--check`), and it renders offline in Chromium with the network blocked (3 stylesheets applied, composer wired, no page errors). Falsified by touching `ui/styles.css` -- the drift check fails. |
...
| Non-speech guard (the "thank you" bug) | Whisper hallucinates stock phrases on silence/non-speech. Peak gate (-50 dBFS) + density/artifact check, both directions pinned | `tests/plugins/test_mockingbird_transcriber.py` (CLI repo); 8/8 live matrix |
| Studio (recording library) | Takes append, transcripts edit in place, reorder, splice via ffmpeg, lock refuses further edits, **and unlock restores editing** (regression: the summary flush used to throw on a locked record and strand it locked forever) | `recordings.rs` unit tests (incl. real-audio splice); `test_desktop_ui.py` §14e (lock → unlock → edit accepted) |
| Studio owner PIN (lock authority) | First lock sets a PIN (Keychain); unlocking requires it; wrong PIN refused; a locked record edited on disk reads back `tampered`; with no PIN, lock stays free UI state | `studio_auth.rs` unit tests (PIN round-trip, 0600 blob, tag detect); `recordings.rs` (`pin_gates_unlock_when_set`, `out_of_band_edit_is_detected`, `no_pin_keeps_unlock_free`); `test_desktop_ui.py` §14e PIN modal + tamper banner |
| Dictation transcript is ANSI-clean | `--transcribe` prepends the same OSC boot palette as `--version`; the prompt must not show raw escapes | `main.rs` `transcribe_output_is_ansi_free`; `strip_ansi` unit tests |
| Waveform renders under the packaged CSP | `drawWaveform` must not re-fetch the blob: URL (packaged `connect-src` blocks it); it decodes the held bytes | `test_desktop_ui.py` §14e (real-tone mock → canvas assertion) |
...
| Tauri capability surface | **guarded in the harness**: `cap_guard()` reads `capabilities/default.json` and fails the suite if the window set widens beyond `["main"]` or any permission leaves the explicit allowlist (`core:default`) or uses a wildcard. The main window only calls `core` (invoke) + `event` (listen), so `core:default` is the whole surface it needs — a future `fs:`/`shell:`/`http:` grant can't slip in unnoticed | static assertion in `test_desktop_ui.py` |
